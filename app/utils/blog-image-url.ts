export type BlogImageVariant = "cardDesktop" | "cardMobile" | "post" | "og";
...
export function normalizeBlogImageUrlForDpr(
  imageUrl: string,
  variant: BlogImageVariant,
  dpr: 1 | 2
) {
...
export function normalizeBlogImageUrl(
  imageUrl: string,
  variant: BlogImageVariant
) {
...
export function getBlogImageSourceUrl(imageUrl: string) {
  let parsedUrl: URL;
  try {
    parsedUrl = new URL(imageUrl);
  } catch {
